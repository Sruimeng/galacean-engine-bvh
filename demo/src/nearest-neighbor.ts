...
function performNearestSearch(): void {
  ...
  for (let i = 0; i < queryPoints.length; i++) {
    ...
    if (nearest) {
      ...
      point.nearestObject.material.baseColor.set(highlightColor.r, highlightColor.g, highlightColor.b, 1);
  ...
  if (avgDistanceEl) avgDistanceEl.textContent = foundCount > 0 ? (totalDistance / foundCount).toFixed(2) : '-';
  if (minDistanceEl) minDistanceEl.textContent = minDistance < Infinity ? minDistance.toFixed(2) : '-';
}
...
function setupEventListeners(): void {
  ...
  if (maxDistanceEl) {
    maxDistanceEl.addEventListener('input', (e) => {
      ...
      // 更新范围可视化
      for (const point of queryPoints) {
        point.rangeEntity.transform.setScale(config.maxDistance, config.maxDistance, config.maxDistance);
      }
    });
  }
