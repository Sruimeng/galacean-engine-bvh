...
async function runStressTest(): Promise<void> {
  ...
  try {
    ...
    log(`光线投射: ${raycastTime.toFixed(0)}ms, 命中率 ${(hitCount/config.raycastCount*100).toFixed(1)}%`, 'success');
    ...
    log(`范围查询: ${rangeTime.toFixed(0)}ms, 平均找到 ${(totalFound/config.rangeQueryCount).toFixed(1)} 个`, 'success');
